...
function saveTags(tags) {
  ...
  return Tags.forge().query('whereIn', 'slug', _.pluck(tagObjects, 'slug')).fetch().then(function (existingTags) {
    var doNotExist = [];
...
.get(function (req, res) {
  Posts.forge().fetch().then(function (collection) {
...
// fetch a post by id
.get(function (req, res) {
  Post.forge({ id: req.params.id }).fetch({ withRelated: ['categories', 'tags'] }).then(function (post) {
    if (!post) {
...
router.route('/posts').post(function (req, res) {
  ...
  Post.forge({
    ...
    saveTags(tags).then(function (ids) {
      ...
      post.load(['tags']).then(function (model) {
...
router.route('/posts/category/:id').get(function (req, res) {
  Category.forge({ id: req.params.id }).fetch({ withRelated: ['posts'] }).then(function (category) {
...
router.route('/posts/tag/:slug').get(function (req, res) {
  Tag.forge({ slug: req.params.slug }).fetch({ withRelated: ['posts'] }).then(function (tag) {
